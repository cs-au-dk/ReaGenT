...
///////////////////////////////////////////////////////////////////////////////////
//////// COPY PASTE FROM TAJS TO AVOID HAVING TO HAVE TAJS TESTS AS DEPENDENCIES
///////////////////////////////////////////////////////////////////////////////////

public class TajsMisc {

    private static final boolean RECREATE_EXPECTED_OUTPUT = false; // for quick replacement of all expected outputs

    private static Logger log = Logger.getLogger(TajsMisc.class);
    ...
    private static String getMethodName() {
        ...
        for (int i = s.length - 1; i >= 0; i--) {
            if (s[i].getClassName().startsWith("dk.brics.tajs.test") && !s[i].getClassName().equals("dk.brics.tajs.test.TajsMisc")) {
    ...
    public static void checkSystemOutput() {
        ...
        ps = null;
        try {
            TajsMisc.checkOutput(fix(os.toString("UTF-8")));
        } catch (UnsupportedEncodingException e) {
            throw new AnalysisException(e);
        }
        unfixLocale();
