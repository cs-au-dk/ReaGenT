...
public class MiscTajsUtils {

    private static Logger log = Logger.getLogger(MiscTajsUtils.class);
    ...
    public static void run(String[] args, IAnalysisMonitoring monitoring) throws AnalysisException {
        try {
            Analysis a;
            Path main = Paths.get(args[args.length - 1]);
            try {
                a = Main.init(args, monitoring, null);
            } catch (ParseError e) {
                throw e;
            }
    ...
    public static void checkSystemOutput() {
        ps.close();
        try {
            MiscTajsUtils.checkOutput(fix(os.toString("UTF-8")));
        } catch (UnsupportedEncodingException e) {
            throw new AnalysisException(e);
        }
        unfixLocale();
    ...
    public static void runSource(String suffix, String[] src, IAnalysisMonitoring monitoring) {
        try {
            ...
            if (monitoring == null) {
                MiscTajsUtils.run(args);
            } else {
                MiscTajsUtils.run(args, monitoring);
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
