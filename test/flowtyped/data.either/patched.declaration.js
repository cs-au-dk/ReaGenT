...
interface Validation<F, S> extends Applicative<S> {// <- F was unused in original?
    // fold: ???
}
...
declare module 'data.either' {
    ...
    declare class Either<L, A> {
        ...
        getOrElse(v: A): A;
        merge(): A | L;
        ap<B>(fab: Either<L, B> | Applicative<B>): Either<L, B>; // What if this is a Left, and the argument is an Applicative, then the result is an Applicative.
        ...
        static fromNullable<A>(r: ?A): Either<null | void, A>;
        // static fromValidation<L, A>(m: Validation<L, A>): Either<L, A>; // When the validation type is fixed.
    }

    declare module.exports: typeof Either;
}
// TODO: Remove check on Left and Right.