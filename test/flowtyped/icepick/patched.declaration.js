...
declare module 'icepick' {
    ...
    // More benign errors in this.
   /* declare function updateIn<V>(
        collection: V[],
        path: (number | string)[],
        callback: (value: V) => V,
    ): V[];
    ...
    // declare function merge<T, S1>(target: T, source: S1, resolver?: (target: any, source: any, key: string) => any): T & S1; // <- returning "any" can result in a benign error. // TODO: Comment in.
    ...
    declare function map<T, U>(fn: (value: T, index: number, array: T[]) => U, array: T[]): U[];
    declare function filter<T>(fn: (value: T, index: number, array: T[]) => boolean, array: T[]): T[];
    ...
    declare interface IcepickArrayWrapper<T> {
        value(): T[]; // benign: any can sneak into the values.
        ...
        map<U>(fn: (value: T, index: number, array: T[]) => U): IcepickArrayWrapper<U>; // TODO: See below.
        filter(fn: (value: T, index: number, array: T[]) => boolean): IcepickArrayWrapper<T>; // TODO: Benign, value might be null, for some reason....
